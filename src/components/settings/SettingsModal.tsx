...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleTestApiKey = async () => {
    if (!apiKeyInput.trim()) {
      setKeyTestStatus({ ok: false, msg: 'Please enter a Gemini API Key first.' });
      return;
    }
    setTestingKey(true);
    setKeyTestStatus(null);
    try {
      geminiClient.setClientApiKey(apiKeyInput.trim());
      const testRes = await geminiClient.executeQuickAction({
        prompt: 'Say "Nexora AI Connected Successfully!" in 5 words.',
        action: 'explain'
      });
      if (testRes) {
        setKeyTestStatus({ ok: true, msg: 'API Key Verified! AI engine is ready to generate responses.' });
      } else {
        setKeyTestStatus({ ok: false, msg: 'Received empty response. Please verify your key.' });
      }
    } catch (err: any) {
      setKeyTestStatus({ ok: false, msg: err.message || 'Verification failed. Please check key validity.' });
    } finally {
      setTestingKey(false);
    }
  };
            ...
            <div className="space-y-4">
              ...
              <div className="mt-6 pt-5 border-t border-slate-800">
                ...
                <p className="text-[11px] text-slate-400 mb-3 leading-relaxed">
                  If your live deployment is running as a static site (e.g. Netlify/Vercel) without a proxy server, you can save your Gemini API Key here. It stays securely in your local browser storage.
                </p>
                ...
                <div className="space-y-2">
                  <div className="relative flex items-center">
                    <input
                      ...
                      placeholder="AIzaSy..."
                      className="w-full bg-slate-900 border border-slate-700/80 rounded-xl px-3.5 py-2.5 text-xs text-slate-100 placeholder-slate-500 focus:outline-hidden focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 pr-20 font-mono"
                    />
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2 pt-1">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {apiKeySavedSuccess ? <Check className="w-3.5 h-3.5 text-emerald-300" /> : <Key className="w-3.5 h-3.5" />}
                        <span>{apiKeySavedSuccess ? 'Saved!' : 'Save Key'}</span>
                      </button>
                      ...
                      <button
                        ...
                        disabled={testingKey}
                        className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-200 text-xs font-medium flex items-center gap-1.5 transition-colors disabled:opacity-50"
                      >
                        {testingKey ? (
